...
const ProfileDashboard: React.FC<{ user: any, creatorIdOverride?: string, isNetworkLevel?: boolean, onAuthRequest?: () => void }> = ({ user, creatorIdOverride, isNetworkLevel, onAuthRequest }) => {
  ...
  const [activeTab, setActiveTab] = useState<'feed' | 'store' | 'live' | 'booking' | 'series' | 'courses' | 'wallet' | 'flipbook' | 'appearance' | 'my_bookings' | 'networks' | 'members' | 'community' | 'security' | 'crm' | 'subscriptions' | 'whatsapp'>(() => {
    ...
    // If it is a child network under the Bonaire parent N2N network, default to 'store'
    if (isNetworkLevel && wlConfig?.parent_network_id === 'b0ea0000-c08f-4260-8540-a0cc8bed4e11') {
      return 'store';
    }
    return 'feed';
  });
  ...
  if (!profile && isCourtney) {
    const courtneyFallback = {
      id: 'c0071234-c08f-4260-8540-a0cc8bed4e11',
      username: 'The Real Courtney Bee',
      bio: 'Courtney Bee is a star on the NYC comedy scene (Wild \'N Out Season 18, HBO Max "That Damn Michael Che"). With her hometown Detroit roots, aggressive punchlines, and hilarious wit, she opens for Michael Che and Dulcé Sloan nationwide.',
      avatar_url: 'https://static.wixstatic.com/media/066ffc_bb9bdff854db4b56bb3f6b58ee1ce532~mv2.png/v1/crop/x_0,y_261,w_1242,h_763/fill/w_860,h_528,al_c,q_90,usm_0.66_1.00_0.01,enc_avif,quality_auto/image%20(1).png',
    ...
    setProfile(courtneyFallback);
    setBio(courtneyFallback.bio);
    setAvatarUrl(courtneyFallback.avatar_url);
        ...
        {isOwnProfile && (
          <div style={{ padding: '12px', display: 'flex', justifyContent: 'center', position: 'relative', zIndex: 100, marginBottom: '20px' }}>
            <div style={{ display: 'flex', gap: isMobile ? '10px' : '8px', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(12px)', borderRadius: '30px', padding: '5px', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
              ...
              >
                <Edit2 size={16} color={viewMode === 'edit' ? '#000' : '#fdd835'} style={{ filter: viewMode === 'edit' ? 'drop-shadow(0 1px 2px rgba(255,255,255,0.9))' : 'none' }} /> Edit Channel
              </button>
              <button 
                onClick={() => {
                  setViewMode('public');
              ...
              >
                <Eye size={16} color={viewMode === 'public' ? '#fff' : '#64b5f6'} /> Public Preview
              </button>
            </div>
          </div>
        )}
                      ...
                      return (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                          ...
                          >
                            ...
                            {/* Conditional Bio Pic Overlay in Edit Mode */}
                            {isOwnProfile && viewMode === 'edit' && (
                              effectiveAvatar ? (
                                /* Case A: Custom Bio Pic exists -> Overlay ONLY the suggested size badge over bottom edge */
                                <div 
                                  style={{
                                    position: 'absolute',
                                    bottom: '10px',
                                ...
                                /* Case B: No Bio Pic set -> Full 144px Change Photo & Suggested Size overlay */
                                <div 
                                  style={{
                                    position: 'absolute',
                                    top: '-2px',
                          ...
                          <div className="profile-badge-container" style={{ display: 'flex', justifyContent: 'center' }}>
                            <span style={{ padding: '6px 14px', background: `${wlConfig?.accent || '#ff4d85'}22`, color: wlConfig?.accent || '#ff4d85', border: `1px solid ${wlConfig?.accent || '#ff4d85'}44`, borderRadius: '24px', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', whiteSpace: 'nowrap' }}>
                              Verified Channel
                            </span>
                          </div>
                        </div>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
              ...
              <div style={{ position: 'relative', width: '100%', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <textarea 
                  placeholder={`What's on your mind, ${profile?.full_name || profile?.username || 'Creator'}?`} 
                  value={postTitle} 
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ background: 'rgba(255,255,255,0.01)', borderRadius: '24px', padding: 'clamp(14px, 4vw, 30px)', border: '1px solid rgba(255,255,255,0.04)', backdropFilter: 'blur(20px)' }}>
                ...
                {/* Scheduled Sessions & Client Appointments History (Creator Edit Mode) */}
                {isOwnProfile && viewMode === 'edit' && user && (() => {
                  ...
                  return (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ background: 'rgba(255,255,255,0.01)', padding: '28px', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.08)', marginBottom: '30px', backdropFilter: 'blur(20px)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '20px', marginBottom: '24px' }}>
                        <div>
                          <h4 style={{ margin: '0 0 4px 0', fontSize: '18px', display: 'flex', alignItems: 'center', gap: '8px', color: wlConfig?.accent || '#00ff88', fontWeight: 'bold' }}>
                            <Calendar size={20} /> Scheduled Appointments & Client Sessions ({allBookings.length})
                          </h4>
                          <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '13px' }}>View upcoming client calls, host virtual sessions, and download call recordings.</p>
                        </div>
                      </div>

                      {allBookings.length === 0 ? (
                        <p style={{ color: 'var(--text-muted)', fontStyle: 'italic', margin: 0 }}>You have no scheduled client appointments yet.</p>
                      ) : (
                ...
                {!(isOwnProfile && viewMode === 'edit') && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', alignItems: 'start' }}>
                      {/* Mobile Dropdowns Card (shown on mobile <= 900px) */}
                      <div className="show-on-mobile-block" style={{ background: 'rgba(255,255,255,0.015)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '20px', padding: 'clamp(16px, 4vw, 24px)', backdropFilter: 'blur(20px)', flexDirection: 'column', gap: '16px' }}>
                      ...
                      {/* Desktop Left Column: Calendar & Time Slots (shown on desktop > 900px) */}
                      <div className="hide-on-mobile-block" style={{ flexDirection: 'column', gap: '20px' }}>
