      ...
      <div className="gap-mobile-sm" style={{ display: 'flex', alignItems: 'center', gap: '60px' }}>
        <Link to={`/${window.location.search}`} style={{ textDecoration: 'none' }}>
          {appLogo ? (
            ...
            <h1 style={{ margin: 0, fontSize: '24px', color: appAccent || '#fff', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '2px', fontFamily: "'RNS Miles', sans-serif" }}>{appName}</h1>
          ) : (
            <img 
              src={ASSETS.logo} 
